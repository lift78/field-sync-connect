...
export const dbOperations = {
  ...
  async clearSyncedRecords() {
    const [cashCount, loanAppCount, disbursementCount, advanceCount] = await Promise.all([
    ...
    return {
      ...
      loanDisbursements: disbursementCount,
      advanceLoans: advanceCount,
      total: cashCount + loanAppCount + disbursementCount + advanceCount
    };
  },

  // Delete pending records older than 3 days
  async deleteOldPendingRecords() {
    const threeDaysAgo = new Date();
    threeDaysAgo.setDate(threeDaysAgo.getDate() - 3);
    
    const [cashCount, loanAppCount, disbursementCount, advanceCount] = await Promise.all([
      db.cashCollections.filter(record => 
        record.synced === false && record.timestamp <= threeDaysAgo
      ).delete(),
  ...
  // Get count of old pending records (for UI display)
  async getOldPendingRecordsCount() {
    const threeDaysAgo = new Date();
    threeDaysAgo.setDate(threeDaysAgo.getDate() - 3);
